...
def collect_image_paths(data_dir):
    ...
    image_keys = [structure.BrainImageTypes.T1w, structure.BrainImageTypes.GroundTruth]

    class MyFilePathGenerator(futil.FilePathGenerator):
        @staticmethod
        def get_full_file_path(
            id_: str, root_dir: str, file_key, file_extension: str
        ) -> str:
...
def load_images(image_paths):
    ...
    image_dict = {
        structure.BrainImageTypes.T1w: sitk.ReadImage(
            fileName=image_paths[structure.BrainImageTypes.T1w],
            outputPixelType=sitk.sitkFloat32,
        ),
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(
            fileName=image_paths[structure.BrainImageTypes.GroundTruth],
...
def register_images(image_dict, atlas_img):
    ...
    registered_t1 = registration.execute(
        list(image_dict.values())[0], params=registration_params
    )
    ...
    Path("registered_gts").mkdir(parents=True, exist_ok=True)
    sitk.WriteImage(registered_gt, "registered_gt.nii.gz")
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    class MinMaxRescaleFilter(fltr.Filter):
        def execute(
            self, img: sitk.Image, params: MinMaxRescaleFilterParams = None
        ) -> sitk.Image:
            rescaled_img = sitk.RescaleIntensity(img, params.min, params.max)
            return rescaled_img

    filter_ = MinMaxRescaleFilter()
    filter_params = MinMaxRescaleFilterParams(min_=new_min_val, max_=new_max_val)
    minmax_rescaled_img = filter_.execute(
        img=list(image_dict.values())[0], params=filter_params
    )
...
def extract_feature_median_t1(image_dict):
    ...
    class MedianFilter(fltr.Filter):
        def execute(
            self, img: sitk.Image, params: fltr.FilterParams = None
        ) -> sitk.Image:
...
if __name__ == "__main__":
    ...
    atlas_img_ = sitk.ReadImage(
        "../data/exercise/mni_icbm152_t1_tal_nlin_sym_09a.nii.gz"
    )
    if isinstance(subject_x_paths, dict):
