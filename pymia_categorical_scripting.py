...
class SymmetricBoundaryDice(ConfusionMatrixMetric):
    
    def __init__(self, footprint = 5, metric: str = 'SBDICE'):
        '''
        Represents a symmetric boundary dice metric.
        Metric with hybrid features, based on the concept of boundary overlap.
        Borrows features from both overlap-based and boundary-distance-based metric.
        Boundary overlap metrics quantify the overlap percentage along the boundaries of the two regions.

        Args:
            metric (str): The identification string of the metric.

        See Also:
            - Yeghiazaryan, Varduhi, Family of boundary overlap metrics for the evaluation of medical image segmentation, Journal of Medical Imaging (2018), 1, 5(01)
        '''
        super().__init__(metric)
        self.footprint = int(footprint)

    def directional_boundary_dice(self):
        # calculate DBD
        edge_min = (sp.ndimage.minimum_filter(ConfusionMatrixMetric.tp, size=self.footprint)!=ConfusionMatrixMetric.tp)
        edge_max = (sp.ndimage.maximum_filter(ConfusionMatrixMetric.tp, size=self.footprint)!=ConfusionMatrixMetric.tp)
        edge_combined = np.logical_or(edge_min, edge_max)

        edge_tp = np.where(edge_combined, ConfusionMatrixMetric.tp, 0)
        edge_tp_idx = np.argwhere(edge_tp)
        edge_tp_n = len(edge_tp_idx)
        
        
        ground_truth = []
        prediction = []
        dice =  []
        for i, dA in enumerate(edge_tp_idx):
            # add here subsets
            print(dA)

        DBD_dice_sum = np.sum(dice.flatten()) # test if it really works like in keras.backend
        DBD = DBD_dice_sum / edge_tp_n
        DBD_sum = 0
        n_edge_points = 0
        return DBD, DBD_sum, edge_tp_n
        
    def calculate(self, y_true, y_pred):
        true_DBD, true_sum_DBD, true_n_edge_points = self.directional_boundary_dice(y_true, y_pred)
        pred_DBD, pred_sum_DBD, pred_n_edge_points = self.directional_boundary_dice(y_pred, y_true)
